...
fn test_extract_from_iter() {
    ...
    let result = Python::with_gil(|py| {
        ...
        pyxirr::xirr(py, dates_iter, Some(amounts_gen), None, None).unwrap().unwrap()
    });
...
fn test_extract_from_tuples() {
    ...
    let result = Python::with_gil(|py| {
        ...
        pyxirr::xirr(py, cash_flow, None, None, None).unwrap().unwrap()
    });
    assert_almost_eq!(result, xirr_expected_result(input));
}
...
fn test_extract_from_dict() {
    ...
    let result = Python::with_gil(|py| {
        ...
        pyxirr::xirr(py, data, None, None, None).unwrap().unwrap()
    });
    assert_almost_eq!(result, xirr_expected_result(input));
}
...
fn test_extract_from_numpy_arrays() {
    ...
    let result = Python::with_gil(|py| {
        ...
        pyxirr::xirr(py, dates, Some(amounts), None, None).unwrap().unwrap()
    });
...
fn test_extract_from_pandas_series_with_datetime_index() {
    ...
    let result = Python::with_gil(|py| {
        ...
        let dates = py
            ...
            .unwrap();
        pyxirr::xirr(py, dates, None, None, None).unwrap().unwrap()
    });
...
fn test_failed_extract_from_pandas_series_with_int64_index() {
    ...
    Python::with_gil(|py| {
        ...
        let err = pyxirr::xirr(py, dates, None, None, None).unwrap_err();
...
fn test_extract_from_non_float() {
    ...
    let result = pyxirr::xirr(py, dates, Some(amounts), None, None).unwrap().unwrap();
    ...
    let err = pyxirr::xirr(py, dates, Some(amounts), None, None).unwrap_err();
    assert!(err.is_instance::<exceptions::PyTypeError>(py));
}
